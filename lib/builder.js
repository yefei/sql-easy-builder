...
const { Raw, Op, Fn, Quote, QBuilder } = require('./q');
...
class Builder {
    ...
    fields.forEach(item => {
      ...
      if (isPlainObject(item)) {
        Object.keys(item).forEach(k => {
          ...
          // { 'asName': [QBuilder] }
          if (item[k] instanceof QBuilder) {
            const [sql, params] = item[k].build(this);
            _fields.push(this.as(sql, k));
            params.length && this._params.push(...params);
            return;
          }
  ...
  update(table, columns) {
    ...
    this.append(Object.keys(columns).map(k => {
      if (columns[k] instanceof Raw) {
        return `${this.quote(k)} = ${this.quote(columns[k])}`;
      }
      if (columns[k] instanceof Op) {
        const _op = columns[k].build();
        this._params.push(..._op.params);
        return `${this.quote(k)} = ${this.raw(_op.sql)}`;
      }
   ...
   */
  func(name, exp, alias) {
    return new Fn(name, exp).as(alias).build(this);
  }
   ...
   */
  count(column = '*', alias) {
    const count = new Fn('COUNT', column);
    return this.select(alias ? { [alias]: count } : count);
  }
